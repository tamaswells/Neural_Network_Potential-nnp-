...
import tqdm
import sys
from ase2 import EnergyShifter2,return_global
...
species_dict=species_to_tensor.rev_species

try:
    path = os.path.dirname(os.path.realpath(__file__))
except NameError:
    path = os.getcwd()
files= 'my.h5' if len(sys.argv)==1 else sys.argv[1]

dspath = os.path.join(path, files)

batch_size = 254

energy_shifter = EnergyShifter2(None)
...
#print(atomic_properties[0]['species'])
...
if os.path.isfile(latest_checkpoint):
    ...
    #best = torch.load(best_checkpoint)
...
device = torch.device('cpu')
coordinates = torch.tensor([[[-3.7814,0.0011,0.00],
                            [-2.5373, -0.2480,0.00]]],
                           requires_grad=True, device=device)
def species_to_tensors(string):
    return torch.tensor([list(map(lambda x:species_dict[x],string))])
species =species_to_tensors('OO')
...
energy_shifters=0.0
print(energy_shifter.self_energies,species)
new_energy=return_global()
for each_atom in species[0]:
    energy_shifters+=new_energy[each_atom]
...
print('Energy:', energy.item()+energy_shifters)
print('Force:', force.squeeze())


from ase2 import Calculator2
...
atom_coodinate=np.array([[-4.00405421,0.833102417,0.00000000],
                        [-4.20737926,2.04431094,0.00000000],
                        [-2.7936405421,1.04414,0.00000000]])
atoms = Atoms('OOO',
             positions=atom_coodinate,
             pbc=[0, 0, 0])
###############################################################################
# Now let's set up a crystal
#atoms = Diamond(symbol="C", pbc=True)
print(len(atoms), "atoms in the cell")
#io.write('test.cif',atoms,'cif')
###############################################################################
# Now let's create a calculator from builtin models:
#calculator = torchani.models.ANI1ccx().ase()
shift=True
converter=1#units.Hartree
calculator= Calculator2('HCNO', aev_computer,nn,energy_shifter,shift,converter)
#Still using the network
atoms.set_calculator(calculator)
io.write('O3_0.xyz',atoms,'xyz')
###############################################################################
...
io.write('O3.xyz',atoms,'xyz')


atom_coodinate=np.array([[-3.4814,0.0011,0.00],[-2.5373, -0.2480,0.00]])
atoms = Atoms('OO',
             positions=atom_coodinate,
             pbc=[0, 0, 0])
shift=True
...
calculator= Calculator2('HCNO', aev_computer,nn,energy_shifter,shift,converter)
atoms.set_calculator(calculator)
io.write('O2_0.xyz',atoms,'xyz')
print("Begin minimizing...")
opt = BFGS(atoms)
opt.run(fmax=0.001)
io.write('O2.xyz',atoms,'xyz')
